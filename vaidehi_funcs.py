...
def set_screen_white(simulation):
     simulation.screen.bgcolor(255,0,0)
...
def set_rainbow(simulation):
    ...
    blue = sin(0.01 * calc_time + 4) * (color_offset - 4) + color_offset
    simulation.screen.bgcolor(int(red), int(green), int(blue))

def cat_head(simulation):
	simulation.screen.register_shape("cat.gif")
	simulation.turtle.shape("cat.gif")